...
FIGURE_1 : Overall, the figure presents a comparison between OP-RAG and long-context models.

**(a)** The bar chart compares F1 scores of different models. OP-RAG models (blue) show higher F1 scores compared to long-context models (orange), with OP-RAG-48K achieving the highest score.

**(b)** The bar chart shows the average input token count for each model. Long-context models have a significantly higher token count compared to OP-RAG models, with Gemini-1.5-Pro having the highest.
...
Context LLMs. As shown in Figure 4a (The plot shows the F1 score versus the number of retrieved chunks for the EN.QA dataset. The order-preserve RAG consistently outperforms vanilla RAG across all chunks, with F1 scores ranging from about 30 to 47.5.), On En.QA
...
FIGURE_2 : A diagram comparing Vanilla RAG with the proposed order-preserve RAG. It shows a long document divided into 13 chunks, each with a similarity score. Vanilla RAG orders these chunks in descending similarity score, while order-preserve RAG maintains the original document order among the highest-scored chunks.
...
FIGURE_3 : This figure consists of two sub-plots illustrating the influence of context length on performance.

**(a)** The left plot shows the F1 score in relation to context length for EN.QA. Llama3.1-8B shows a peak at shorter context lengths and then declines, while Llama3.1-70B maintains a higher, more stable F1 score across the range.
**(b)** The right plot displays accuracy for EN.MC with context length. Llama3.1-8B shows a peak at a shorter context length, while Llama3.1-70B maintains higher accuracy throughout the context length spectrum.

FIGURE_3 : This figure examines how varying context lengths impact the performance of RAG. It evaluates the outcomes on the En.QA and EN.MC datasets derived from the ∞Bench benchmark. The graph likely illustrates performance metrics across different context lengths.
```
...
Figure 2 (A diagram comparing Vanilla RAG with the proposed order-preserve RAG. It shows a long document divided into 13 chunks, each with a similarity score. Vanilla RAG orders these chunks in descending similarity score, while order-preserve RAG maintains the original document order among the highest-scored chunks.) visualizes the difference between the
...
shown in Figure 3 (This figure examines how varying context lengths impact the performance of RAG. It evaluates the outcomes on the En.QA and EN.MC datasets derived from the ∞Bench benchmark. The graph likely illustrates performance metrics across different context lengths.
...
FIGURE_4 : This figure consists of two sub-plots comparing order-preserve RAG and vanilla RAG.

**(a)** The plot shows the F1 score versus the number of retrieved chunks for the EN.QA dataset. The order-preserve RAG consistently outperforms vanilla RAG across all chunks, with F1 scores ranging from about 30 to 47.5.
**(b)** The plot depicts accuracy versus the number of retrieved chunks for the EN.MC dataset. Order-preserve RAG demonstrates higher accuracy compared to vanilla RAG, with scores stabilizing after 100 retrieved chunks.

irrelevant distractions.
Order-preserve RAG versus vanilla RAG. As
shown in Figure 4 (This figure consists of two sub-plots comparing order-preserve RAG and vanilla RAG.

**(a)** The plot shows the F1 score versus the number of retrieved chunks for the EN.QA dataset. The order-preserve RAG consistently outperforms vanilla RAG across all chunks, with F1 scores ranging from about 30 to 47.5.
**(b)** The plot depicts accuracy versus the number of retrieved chunks for the EN.MC dataset. Order-preserve RAG demonstrates higher accuracy compared to vanilla RAG, with scores stabilizing after 100 retrieved chunks.), when the number of retrieved
...
As shown in Table 1 (| Method                            | EN.QA         |      | EN.MC       |      |
|-----------------------------------|---------------|------|-------------|------|
|                                   | F1 Score      | Tokens | Acc.        | Tokens |
| **Long-context LLM w/o RAG**      |               |      |             |      |
| Llama3.1-70B                      | 34.26         | 117K | 71.62       | 117K |
| GPT4O                             | 32.36         | 117K | 78.42       | 117K |
| Gemini-1.5-Pro                    | 43.08         | 195K | 85.57       | 188K |
| **SELF-ROUTE (Li et al., 2024)**  |               |      |             |      |
| GPT4O                             | 34.95         | 85K  | 77.29       | 62K  |
...
| OP-RAG-48K                        | 47.25         | 48K  | 85.59       | 48K  |), without RAG, LLM takes a
...
self-reflection. As shown in Table 1 (| Method                            | EN.QA         |      | EN.MC       |      |
|-----------------------------------|---------------|------|-------------|------|
|                                   | F1 Score      | Tokens | Acc.        | Tokens |
| **Long-context LLM w/o RAG**      |               |      |             |      |
| Llama3.1-70B                      | 34.26         | 117K | 71.62       | 117K |
...
| OP-RAG-16K                        | 44.43         | 16K  | 84.72       | 16K  |
| OP-RAG-24K                        | 45.45         | 24K  | 88.65       | 24K  |
| OP-RAG-48K                        | 47.25         | 48K  | 85.59       | 48K  |), ours signifi-

TABLE_1 : | Method                            | EN.QA         |      | EN.MC       |      |
|-----------------------------------|---------------|------|-------------|------|
|                                   | F1 Score      | Tokens | Acc.        | Tokens |
| **Long-context LLM w/o RAG**      |               |      |             |      |
| Llama3.1-70B                      | 34.26         | 117K | 71.62       | 117K |
