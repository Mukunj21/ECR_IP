...
[Figure: Two-panel bar chart comparing OP-RAG with long-context LLMs. Panel (a) shows F1 score bars for OP-RAG-16K, OP-RAG-24K, OP-RAG-48K, Llama3.1-70B, GPT-4O, and Gemini-1.5-Pro with values labeled above bars; blue bars represent Llama3.1-70B OP-RAG and orange bars represent Long Context. Panel (b) shows average input token count (K) for the same methods, again with blue and orange bars. Caption below identifies Figure 1 *(Two-panel bar chart. Panel (a) shows F1 score comparisons across methods, with blue bars for OP-RAG… — p.1)* as a comparison of proposed order-preserve retrieval-augmented generation (OP-RAG) against long-context LLMs without RAG on En.QA dataset of ∞Bench, using Llama3.1-70B as generator, and notes that OP-RAG significantly outperforms the counterpart without RAG.]
...
Figure 2 *(A schematic diagram comparing two chunk-ordering strategies for retrieval-augmented generation. The… — p.2)*: Vanilla RAG versus the proposed order-preserve the RAG. As shown in the example, a long document is cropped into 13 chunks, {c_i}_{i=1}^{13}. The similarity score is appended to each chunk. We retrieve top 4 chunks with the highest similarity scores. Vanilla RAG places the chunks in a score-descending order, whereas the proposed order-preserve RAG places the chunks based on the order in the original document.
...
Figure 3 *(Two-panel line chart figure showing the influence of context length (x-axis, in K) on RAG performanc… — p.3)*: The influence of context length on the performance of RAG. The evaluations are conducted on En.QA and EN.MC datasets of ∞Bench.
...
Figure 2 *(A schematic diagram comparing two chunk-ordering strategies for retrieval-augmented generation. The… — p.2)* visualizes the difference between the
...
shown in Figure 3 *(Two-panel line chart figure showing the influence of context length (x-axis, in K) on RAG performanc… — p.3)*, as the context length increases,
...
[Figure: Two-panel line chart figure. Left subplot (a) EN.QA plots F1 score versus number of retrieved chunks, comparing Vanilla RAG (blue circles) and Order-Preserve RAG (orange triangles). Right subplot (b) EN.MC plots Accuracy versus number of retrieved chunks with the same two methods. In both subplots, the order-preserve method is consistently higher, especially at larger chunk counts.]

Figure 4 *(Two-panel line chart figure. Left subplot (a) EN.QA plots F1 score versus number of retrieved chunks… — p.4)*: Comparisons between the proposed order-preserve RAG and vanilla RAG. The evaluations are conducted on En.QA and EN.MC datasets of ∞Bench, using Llama3.1-70B model.

Order-preserve RAG versus vanilla RAG. As shown in Figure 4 *(Two-panel line chart figure. Left subplot (a) EN.QA plots F1 score versus number of retrieved chunks… — p.4)*, when the number of retrieved chunks are small (e.g., 8), the advantage of the proposed order-preserve RAG over vanilla RAG is not considerably. In contrast, when the number of retrieved chunks is large, our order-preserve RAG significantly outperforms vanilla RAG. To be specific, on EN.QA dataset, when the number of retrieved chunk is 128, vanilla RAG only achieves 38.40 F1-score whereas our order-preserve RAG achieves 44.43 F1-score. On EN.MC dataset, retrieving 192 chunks, vanilla RAG only achieves 81.22 accuracy whereas our order-preserve RAG obtains 88.65 accuracy.
...
Grégoire Mialon, Roberto Dessì, Maria Lomeli, Christoforos Nalpantis, Ram Pasunuru, Roberta Raileanu, Baptiste Rozière, Timo Schick, Jane Dwivedi-Yu, Asli Celikyilmaz, et al. 2023. Augmented language models: a survey. *arXiv preprint arXiv:2302.07842.*

## References

Marah Abdin, Sam Ade Jacobs, Ammar Ahmad Awan, Jyoti Aneja, Ahmed Awadallah, Hany Awadalla, Nguyen Bach, Amit Bahree, Arash Bakhtiari, Harikr- at Behl, et al. 2024. Phi-3 technical report: A highly capable language model locally on your phone. *arXiv preprint arXiv:2404.14219.*
...
Krzysztof Choromanski, Valerii Likhosherstov, David Dohan, Xingyou Song, Andreea Gane, Tamar Sarlos, Peter Hawkins, Jared Davis, Afroz Mohiuddin, Lukasz Kaiser, et al. 2020. Rethinking attention with performers. *arXiv preprint arXiv:2009.14794.*
